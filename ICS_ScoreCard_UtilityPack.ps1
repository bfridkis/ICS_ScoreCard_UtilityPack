...
$patcheckerOutputFileName = "C:\Users\admbfridkis\desktop\customtools\PatchCheckerOutput_$(if ((Get-Date).Month -eq 1) { "12_$((Get-Date).Year - 1)" } else { "$($(Get-Date).Month - 1)_$((Get-Date).Year)" })"
$stalePasswordCheckerOutputFileName = "C:\Users\admbfridkis\desktop\customtools\StalePasswordCheckerOutput_$(if ((Get-Date).Month -eq 1) { "12_$((Get-Date).Year - 1)" } else { "$($(Get-Date).Month - 1)_$((Get-Date).Year)" }).csv"
$serviceCheckerOutputFileName = "C:\Users\admbfridkis\desktop\customtools\ServiceCheckerOutput_$(if ((Get-Date).Month -eq 1) { "12_$((Get-Date).Year - 1)" } else { "$($(Get-Date).Month - 1)_$((Get-Date).Year)" }).csv"
