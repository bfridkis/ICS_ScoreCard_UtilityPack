...
$serviceCheckerJob = Start-Job { 
                                  E:\ICSScoreCardUtilityPack\ServiceCheckerV1.1.ps1 -Services 'SEPMasterService McShield' -CheckAllNodes `
...
$serviceCheckerOutputFileName = "E:\ICSScoreCardUtilityPack\Results\ServiceCheckerOutput_$(if ((Get-Date).Month -eq 1) { "12_$((Get-Date).Year - 1)" } else { "$($(Get-Date).Month - 1)_$((Get-Date).Year)" }).csv"
